...
class DeepHeatmapsModel(object):
    ...
    def create_summary_ops(self):
            ...
            if self.log_histograms:
                self.batch_summary_op = tf.summary.merge([l_total, var_summary, grad_summary,
                                                          activ_summary])
            else:
                self.batch_summary_op = tf.summary.merge([l_total])
    ...
    def train(self):
            ...
            with tf.Session(config=self.config) as sess:
                ...
                for step in range(self.train_iter + 1):
                    ...
                    batch_images, _, batch_maps_small, _ =\
                        load_data(self.img_menpo_list, batch_inds, image_size=self.image_size, c_dim=self.c_dim,
                                  num_landmarks=self.num_landmarks, sigma=self.sigma, scale=self.scale,
                                  save_landmarks=False, primary=True)
                    ...
                    if step == 0 or (step + 1) % self.print_every == 0:
                        ...
                        summary, l_t = sess.run([self.batch_summary_op, self.total_loss], feed_dict_train)

                        summary_writer.add_summary(summary, step)

                        print ('epoch: [%d] step: [%d/%d] primary loss: [%.6f]' % (epoch, step + 1, self.train_iter, l_t))
